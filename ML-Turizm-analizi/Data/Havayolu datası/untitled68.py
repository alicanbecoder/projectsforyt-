# -*- coding: utf-8 -*-
"""
Created on Tue Oct 28 02:34:19 2025
...
# Liste elemanlarındaki boşlukları temizle
...
# Fonksiyon Tanımı: DHMİ Kümülatif Verilerini Okur, Filtreler, Birleştirir
def dhmi_12_ay_birlestir_filtreli(klasor_yolu, dosya_kalibi="YOLCU_*.xlsx", hedef_liste=None):
    ...
    for dosya_yolu in dosya_listesi:
        ...
        # print(f"İşleniyor: {dosya_adi}") # Daha az çıktı için yorum satırı
        try:
            # --- Ay Numarasını Al ---
            ...
            if df.shape[1] <= dis_hat_index:
                 print(f"HATA: '{dosya_adi}' dosyasında beklenen Dış Hat sütunu (index {dis_hat_index}) yok.")
                 continue
            ...
            if df_filtrelenmis.empty:
                # print(f"-> Uyarı: '{dosya_adi}' dosyasında hedef havalimanlarından hiçbiri bulunamadı.") # Çok fazla çıktı üretebilir
                continue 
    ...
    try:
        # Hedef havalimanları listesini index olarak kullan
...
# --- FONKSİYONU ÇAĞIRMA VE AYLIK VERİYİ HESAPLAMA ---

# 1. Klasör Yolunu Belirt
excel_klasor_yolu = r"C:\Users\Alican\Desktop\Çalışma grubu\Havayolu datası\2023\pdf24_convertPdfTo (1)" 
# excel_klasor_yolu = r"C:\Users\Alican\Desktop\Çalışma grubu\Havayolu datası\2023\pdf24_convertPdfTo (1)" # Diğer yıl için

# 2. Dosya Kalıbını Belirt
aranacak_kalip = "YOLCU_2023_*.xlsx" 
# aranacak_kalip = "YOLCU_2023_*.xlsx" # Diğer yıl için

# --- YILI DOSYA KALIBINDAN OTOMATİK AL ---
yil_eslesme = re.search(r'(\d{4})', aranacak_kalip) 
if yil_eslesme:
    islenen_yil_str = yil_eslesme.group(1) 
else:
    islenen_yil_str = "BILINMEYEN_YIL" 
print(f"-> İşlenecek Yıl: {islenen_yil_str}")

# 3. Kümülatif Veriyi Oluşturmak İçin Fonksiyonu Çağır
...
# 4. Kümülatif Veri Başarıyla Oluşturulduysa Aylık Veriyi Hesapla
if kumulatif_filtrelenmis_df is not None:
    ...
    try:
        ...
        for i in range(1, len(sirali_sutunlar)):
            ...
            # print(f"-> {yeni_sutun_adi} hesaplandı.") # Daha az çıktı için
        ...
        # --- Sonucu Göster (Aylık) ---
        print(f"\n--- SONUÇ: {islenen_yil_str} Yılı Hesaplanmış Aylık Dış Hat Yolcu Sayıları (Filtrelenmiş - İlk 10 Satır) ---")
        print(df_sonuc_aylik.head(10).to_markdown(index=True, floatfmt=","))

        # --- Sonucu Kaydet (Aylık) ---
        cikti_dosya_adi = f"SONUC_Aylik_Dis_Hat_{islenen_yil_str}.xlsx"
        cikti_dosya_yolu_aylik = os.path.join(excel_klasor_yolu, cikti_dosya_adi) 
        try:
    ...
    print("\nİşlem başarısız oldu (Kümülatif veri oluşturulamadı).")